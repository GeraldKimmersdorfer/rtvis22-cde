...
rowdropchance = 0 # to gain a smaller filesize for debugging, should be 0 for full dataset

onlycompress = False

filedest = "data/sources/cdata.lzma"
...
random.seed(0) # same seed for same files
...
def compress_file_async_lz4():
    with lz4.frame.open(filedest, "wb", compression_level=lz4.frame.COMPRESSIONLEVEL_MAX, block_size=lz4.frame.BLOCKSIZE_MAX4MB) as fp:
        fp.write(bdata)

if onlycompress:
    print("Reading binary cdata file")
    with open("data/sources/cdata.bin", mode="rb") as file:
        bdata = file.read()

thread = threading.Thread(target=compress_file_async_lzma)
thread.start()
