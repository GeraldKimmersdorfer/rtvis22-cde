...
import { colToString, formatUIntNumbers, getMonthDifference } from "./helper";
...
export const InitUserInterface = () => {
    ...
    var html = '';
    db.DB_files.forEach((itm, ind) => {
        html += `<div id="db_card_${ind}" data-dfid="${ind}" class="db_card"><div class="db_card_title">${itm.name}</div><div class="db_card_info">${itm.temperatures} Temperatures, ${itm.locations} Locations<br>${itm.discretization} discretization, ${itm.size} (uncompressed)</div><div class="db_card_filename">${itm.filename} (${itm.compressedsize})</div></div>`;
    });
    $("#db-cards").html(html);
...
export const refreshGraphInformation = (buff: GridBuffer) => {
    ...
    $("#lbl-aggrn").html(formatUIntNumbers(info.n_total));
    refreshLegend();
}
...
export const showLoadingDialog = () => {
    $("#tblLoading .history").html("");
    $("#spLoading").addClass("animated").html("refresh");
    loadingDialogProgress(0);
    $("body").removeClass("map").addClass("loading");
    $("#loadingDialog").dialog("open");
    $("#canvas-webgpu").fadeOut(800);
}

export const loadedDatabaseChanged = () => {
    $(".db_card").removeClass("selected");
    $("#db_card_" + db.getCurrentDatabseId()).addClass("selected");
}